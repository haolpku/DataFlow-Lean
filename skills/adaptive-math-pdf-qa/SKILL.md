...
3. Run `scripts/book_inspector.py`. Open every image listed in `inspection/manifest.json` and inspect
   ...
   appendices, and answer/hint sections. Native PDF text is secondary evidence.
4. Read [profile-schema.md](references/profile-schema.md), then edit only `book_profile.json` to
   ...
   high-risk glyphs. Preserve the all-environment taxonomy unless the user narrows it.
...
6. Read [direct-route.md](references/direct-route.md), then run:
   ...
   The pipeline checkpoints inventory and extraction windows, audits numbering, retries only
   missing/incomplete/conflicting targets, builds the dataset, and validates it. Resume normally;
